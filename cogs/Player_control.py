...
class MasterCommandGroup(app_commands.Group, name="진행자", description="게임 진행과 관련된 명령어 모음입니다."):
    ...
    async def coin_manage(self, interaction: discord.Interaction, 이름: str, 코인: int):
        target_status = None
        for status in interaction.client.player_status:
            if status.name == 이름:
                target_status = status
    ...
    async def round_score_manage(self, interaction: discord.Interaction, 이름: str, 점수: int):
        target_status = None
        for status in interaction.client.player_status:
            if status.name == 이름:
                target_status = status
    ...
    async def round_multiplier_manage(self, interaction: discord.Interaction, 이름: str, 배수: int):
        target_status = None
        for status in interaction.client.player_status:
            if status.name == 이름:
                target_status = status
